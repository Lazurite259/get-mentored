      ...
      <table class="table table-striped">
        ...
        <tbody>
          <tr v-for="mentor in Mentors" :key="mentor._id">
            ...
            <td>
              <router-link :to="{ name: 'mentor-profile', params: { id: mentor._id } }" class="btn btn-success">Edit
              </router-link>
...
export default {
  ...
  methods: {
    deleteMentor (id) {
      const apiURL = `http://localhost:4000/api/delete-mentor/${id}`
      const indexOfArrayItem = this.Mentors.findIndex(i => i._id === id)
